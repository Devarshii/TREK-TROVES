...
<html>
  ...
  <body onload="submitPayuForm()">
    ...
    if ((int)$orderquery->rowCount() <= 0) {
      ...
      <form action="<?php echo $action; ?>" method="post" name="payuForm">
        ...
        <input type="hidden" name="amount" value="<?php echo $results->totalPrice; ?>" />
        <input type="hidden" name="firstname" id="firstname" value="<?php echo $results->full_name ?>" />
        <input type="hidden" name="email" id="email" value="<?php echo  $results->email ?>" />
        <input type="hidden" name="phone" id="phone" value="<?php echo  $results->contact_number ?>" />
        ...
        <input type="hidden" name="udf1" value="<?php echo $param_array['order_id']?>"   />
  ...
  function getCallbackUrl($type, $param_array) {
    if($type == 1){
    $params = base64_encode("type=1&oid=" . $param_array['order_id']);
      $pageLink = "place_order.php?get_param=" . $params;
    }else{
        $params = base64_encode("type=2&oid=" . $param_array['order_id']);
      $pageLink = "place_order.php?get_param=" . $params;
    }
