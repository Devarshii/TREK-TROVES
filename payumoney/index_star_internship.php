...
<html>
  ...
  <body onload="submitPayuForm()">
    <?php  
    $projectDetailsQuery = select_query('star_internship_courses', '*', 'id = "' . $param_array['course_id'] . '"');
    $studentDetailsQuery = select_query('star_internship_registration', '*', 'id = "' . $param_array['s_id'] . '"');
    ...
    if (mysqli_num_rows($projectDetailsQuery) <= 0   || mysqli_num_rows($studentDetailsQuery) <= 0 ) {
      ...
      <form action="<?php echo $action; ?>" method="post" name="payuForm">
        ...
        <input type="hidden" name="email" id="email" value="abc@gmail.com" />  
  ...
  function getCallbackUrl($type,$param_array) {
    ...
    $params = base64_encode("type=1&oid=" . $param_array['order_code'] . "&s_id=" . $param_array['s_id']);
      $pageLink = "star_internship_success.php?get_param=" . $params;
    }else{
